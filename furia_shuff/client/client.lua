...
--------------------------------MENU DRZWI POJAZDU--------------------------------------------
RegisterNetEvent("kapi:pojazdy")
AddEventHandler("kapi:pojazdy", function()
	CarInteractionMenu()
end)
...
function changeseat(seat)
	if IsPedInAnyVehicle(PlayerPedId(), false) then
		...
		if IsVehicleStopped(car) and not changing then
			--TriggerEvent("pNotify:SendNotification", {text = "Przesiadasz się...", type = "sucess",queue = "global",timeout = 3000,layout = "Right"})
			TriggerEvent("pNotify:SendNotification", {text = 'Przesiadasz się...', timeout=7000, layout="BottomRight"})
			changing = true
			Wait(3000)
